...
const scannedPdfError = (): ResumeParseError =>
  createResumeParseError({
    code: 'UNSUPPORTED_SCANNED_PDF',
    format: 'pdf',
    message:
      'This PDF appears to be scanned or image-only. Please upload a text-based PDF or DOCX file.',
    guidance: [
...
const encryptedPdfError = (): ResumeParseError =>
  createResumeParseError({
    code: 'UNSUPPORTED_ENCRYPTED_PDF',
    format: 'pdf',
    message:
      'This PDF is encrypted or password-protected. Please remove the password and upload an unencrypted text PDF or DOCX file.',
    guidance: [
...
const invalidPdfError = (): ResumeParseError =>
  createResumeParseError({
    code: 'INVALID_PDF',
    format: 'pdf',
...
const isEncryptedPdfBuffer = (buffer: Uint8Array): boolean =>
  Buffer.from(buffer).includes(encryptedPdfMarker);

const extractPageText = async (documentProxy: Awaited<ReturnType<typeof getDocument>['promise']>): Promise<string> => {
  const pages: string[] = [];
...
export const parseTextPdf = async (buffer: Uint8Array): Promise<string> => {
