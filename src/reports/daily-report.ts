...
const compareJobs = (left: DailyReportJob, right: DailyReportJob): number =>
...
const resolveReportJobs = (jobs: readonly JobRecord[]): JobRecord[] => {
...
const toDailyReportJob = (job: JobRecord, profile: CareerProfile): DailyReportJob => {
...
const toCollectedAtValues = (
  reportJobs: readonly DailyReportJob[],
  sourceStatuses: readonly DailyReportSourceStatus[],
): string[] => {
...
const deriveFallbackStatuses = (jobs: readonly JobRecord[]): DailyReportSourceStatus[] => {
  const grouped = new Map<string, DailyReportSourceStatus>();
...
const collectDeadlineAnomalies = (
  reportJobs: readonly DailyReportJob[],
  reportDate: Date,
): { reminders: DeadlineReminder[]; anomalies: string[] } => {
  ...
  reminders.sort(
    (left, right) =>
      left.daysUntilDeadline - right.daysUntilDeadline || compareJobs(left, right),
  );
...
const isSafeJobUrl = (value: string): boolean => {
  try {
...
const formatJobLineItems = (job: DailyReportJob): string[] => [
...
const renderMarkdown = (report: DailyReport): string => {
  const lines: string[] = [
    ...
    '| --- | --- | ---: | ---: | --- | --- |',
    ...report.sourceStatuses.map(
      (status) =>
        `| ${status.source} | ${status.status} | ${status.fileCount} | ${status.jobCount} | ${status.collectedAt.join(', ') || 'N/A'} | ${status.errors.join(' ; ') || 'None'} |`,
    ),
    '',
    '## Failures',
    '',
...
const renderHtml = (report: DailyReport): string => {
  ...
  const failures = report.failures.length === 0
    ...
    : report.failures
        .map(
          (failure) =>
            `<li>${escapeHtml(failure.source)}: ${escapeHtml(failure.filePath)} -> ${escapeHtml(failure.message)}</li>`,
        )
        .join('');
  ...
  const recommendedJobs = report.recommendedJobs.length === 0
    ...
    : report.recommendedJobs
        .map(
          (job) =>
            `<li>${escapeHtml(job.title)} @ ${escapeHtml(job.company)} (${job.matchScore})</li>`,
        )
        .join('');
  ...
  const deadlineReminders = report.deadlineReminders.length === 0
    ...
    : report.deadlineReminders
        .map(
          (job) =>
            `<li>${escapeHtml(job.title)} @ ${escapeHtml(job.company)} (${job.daysUntilDeadline} day(s))</li>`,
        )
        .join('');
...
export const writeReportBundle = async (
  report: DailyReport,
  outputRoot: string,
): Promise<void> => {
