...
import { defaultConfig, type JobHuntingConfig } from './default-config.js';
...
type ConfigInput = Partial<{
  ...
  browserSkill: Partial<{
    enabled: unknown;
    executable: unknown;
    mode: unknown;
    allowedDomains: unknown;
    additionalAllowedDomains: unknown;
    requireUserApproval: unknown;
    maxItemsPerRun: unknown;
    minIntervalMs: unknown;
  }>;
  desktopShortcut: Partial<{
    enabled: unknown;
    name: unknown;
...
const assertBoolean = (value: unknown, field: string, fallback: boolean): boolean => {
  if (value === undefined) {
    return fallback;
  }
...
const assertScheduleTime = (value: unknown, fallback: string): string => {
...
const assertRelativeDirectory = (value: unknown, field: string, fallback: string): string => {
...
export const parseConfig = (input: unknown = {}): JobHuntingConfig => {
  const root = assertObject(input, 'config') as ConfigInput;
  ...
  const allowedDomains = allowedDomainsInput.map((domain, index) =>
    assertNonEmptyString(domain, `browserSkill.allowedDomains[${index}]`, ''),
  );

  const additionalAllowedDomainsInput =
    browserSkill.additionalAllowedDomains ?? defaultConfig.browserSkill.additionalAllowedDomains;
  if (!Array.isArray(additionalAllowedDomainsInput)) {
    throw new TypeError('browserSkill.additionalAllowedDomains must be an array');
  }

  const additionalAllowedDomains = additionalAllowedDomainsInput.map((domain, index) =>
    assertNonEmptyString(domain, `browserSkill.additionalAllowedDomains[${index}]`, ''),
  );
  const resolvedAllowedDomains = [...new Set([...allowedDomains, ...additionalAllowedDomains])];

  const maxItemsPerRun =
    browserSkill.maxItemsPerRun === undefined
      ? defaultConfig.browserSkill.maxItemsPerRun
      : browserSkill.maxItemsPerRun;

  if (
    typeof maxItemsPerRun !== 'number' ||
  ...
  const minIntervalMs =
    browserSkill.minIntervalMs === undefined
  ...
  return {
    ...
    browserSkill: {
      enabled: assertBoolean(
        ...
        'browserSkill.enabled',
        defaultConfig.browserSkill.enabled,
      ),
      executable: assertNonEmptyString(
        browserSkill.executable,
        'browserSkill.executable',
        defaultConfig.browserSkill.executable,
      ),
      mode: 'read-only',
      allowedDomains: resolvedAllowedDomains,
      additionalAllowedDomains,
      requireUserApproval: assertBoolean(
        browserSkill.requireUserApproval,
        'browserSkill.requireUserApproval',
        defaultConfig.browserSkill.requireUserApproval,
      ),
      maxItemsPerRun,
      minIntervalMs,
    },
    desktopShortcut: {
      enabled: assertBoolean(
        ...
        'desktopShortcut.enabled',
        defaultConfig.desktopShortcut.enabled,
      ),
      name: assertNonEmptyString(
        desktopShortcut.name,
        'desktopShortcut.name',
        defaultConfig.desktopShortcut.name,
      ),
      requireApproval: assertBoolean(
        desktopShortcut.requireApproval,
        'desktopShortcut.requireApproval',
        defaultConfig.desktopShortcut.requireApproval,
      ),
    },
  };
};
