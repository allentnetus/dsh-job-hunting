...
- 对齐 GitHub 安装命令、包版本、发布清单和本次版本 tag，避免 `main` 文档继续指向旧交付物。
- 为 DSH 原生 Schedule overlay 提供版本固定的下载入口，并保留会话级提醒和人工确认边界。
- 补充 BrowserSkill 首次运行的 `bsk --help`、`bsk status` 和浏览器扩展检查。
