...
type FakeRegistry<T extends object> = {
  entries: T[];
  register(entry: T): () => void;
};

const createFakeRegistry = <T extends object>(): FakeRegistry<T> => {
  const entries: T[] = [];
  return {
    entries,
    register(entry) {
...
describe('job hunting DSH plugin registration', () => {
  ...
  it('registers one runtime skill and the integrated BrowserSkill tool with fake registries', () => {
    const tools = createFakeRegistry<Record<string, unknown>>();
    const skills = createFakeRegistry<Record<string, unknown>>();
    const ctx = {
      tools,
      skills,
      workspaceRegistry: {
    ...
    const dispose = plugin.apply(ctx as never) as (() => void) | undefined;
