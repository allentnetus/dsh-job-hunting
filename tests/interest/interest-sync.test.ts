...
import type { InterestExport, InterestExportRecord } from '../../src/site/interest-export.js';
import type { InterestState, JobRecord } from '../../src/domain/types.js';
import {
  UnknownInterestJobError,
  InvalidInterestExportError,
  getInterestPool,
  syncInterestExport,
} from '../../src/interest/interest-sync.js';
import {
  InterestConfirmationRequiredError,
  InterestStoreNotConfiguredError,
  createInterestTools,
  createWorkspaceInterestLedgerStore,
  updateInterestFromConversation,
} from '../../src/interest/interest-tools.js';

const baseLedger: InterestState = {
  marks: {
    'job-a': 'interested',
  },
  notes: {
...
describe('interest sync', () => {
  it('duplicate records resolve excluded over interested regardless of input order', () => {
    const interested: InterestExportRecord = {
      jobId: 'job-a',
      mark: 'interested',
      note: '先感兴趣',
      timestamp: exported.updatedAt,
    };
    const excluded: InterestExportRecord = {
      jobId: 'job-a',
      mark: 'excluded',
      note: '明确排除',
      timestamp: exported.updatedAt,
    ...
    for (const records of [[interested, excluded], [excluded, interested]]) {
      const result = syncInterestExport(
        { ...exported, records },
        { marks: {}, notes: {}, updatedAt: baseLedger.updatedAt },
      );
  ...
  it('same-priority records use the same canonical note regardless of input order', () => {
    const first: InterestExportRecord = {
      jobId: 'job-a',
      mark: 'interested',
      note: 'Z 备注',
      timestamp: exported.updatedAt,
    };
    const second: InterestExportRecord = {
      jobId: 'job-a',
      mark: 'interested',
      note: 'A 备注',
      timestamp: exported.updatedAt,
  ...
  it('报告未知岗位 ID，并且不创建虚假标记', () => {
    const invalidExport: InterestExport = {
      ...exported,
      records: [
        ...exported.records,
        {
    ...
    expect(() => syncInterestExport(invalidExport, baseLedger)).toThrowError(
      UnknownInterestJobError,
    );
    try {
  ...
  it.each([
    ...
    ['note', { note: null }],
  ] as const)('malformed %s throws InvalidInterestExportError before ledger mutation', (_, change) => {
    const malformed = {
      ...exported,
      records: [{
        ...exported.records[0],
    ...
    } as unknown as InterestExport;

    expect(() => syncInterestExport(malformed, baseLedger)).toThrowError(
      InvalidInterestExportError,
    );
    expect(baseLedger).toEqual({
      marks: { 'job-a': 'interested' },
...
describe('conversation interest tools', () => {
  ...
  it('confirmed false still rejects markInterest without a confirmation callback', async () => {
    ...
    await expect(
      tools.markInterest({ jobId: 'job-a', mark: 'interested', confirmed: false }),
    ).rejects.toThrowError(InterestConfirmationRequiredError);
  });

  it.each([
    ['number one', 1 as any],
    ['string true', 'true' as any],
    ['missing', undefined],
    ...
    await expect(
      tools.markInterest({ confirmed } as any),
    ).rejects.toThrowError(InterestConfirmationRequiredError);
  ...
  it.each([
    ...
    await expect(
      tools.syncInterest({ exported, confirmed } as any),
    ).rejects.toThrowError(InterestConfirmationRequiredError);
    expect(writes).toBe(0);
  });
  ...
  it('通过显式 Workspace store 持久化并标准化备注与更新时间', async () => {
    ...
    await expect(
      readFile(path.join(workspaceRoot, 'data', 'interest-ledger.json'), 'utf8'),
    ).resolves.toContain('优先 推进');
    await expect(store.read()).resolves.toEqual({
      marks: { 'job-a': 'interested' },
      notes: { 'job-a': '优先 推进' },
      updatedAt: '2026-08-16T10:00:00.000Z',
  ...
  it('没有确认时拒绝把普通对话变成永久兴趣标记', async () => {
    ...
    await expect(tools.updateInterestFromConversation('job-a', 'interested')).rejects.toThrowError(
      InterestConfirmationRequiredError,
    );
  ...
  it('三参数适配器在未配置 Workspace store 时明确失败', async () => {
    await expect(updateInterestFromConversation('job-a', 'interested')).rejects.toThrowError(
      InterestStoreNotConfiguredError,
    );
  });
});
