...
describe('daily report', () => {
    it('拒绝使用未确认的 CareerProfile 构建日报', async () => {
        ...
        expect(() => buildDailyReport([], unconfirmedProfile, '2026-08-16')).toThrow(/profile-confirmed/i);
    });
    it('拒绝不存在的日历日期构建日报', async () => {
        const { buildDailyReport } = await loadReportModule();
        expect(() => buildDailyReport([], confirmedProfile, '2026-02-31')).toThrow(/invalid report date/i);
    });
    it('生成包含新增、推荐、截止提醒、来源状态、失败和异常的可解释日报', async () => {
        ...
        expect(report.sourceStatuses).toEqual(expect.arrayContaining([
            expect.objectContaining({
                source: 'boss',
                status: 'partial',
            }),
        ...
        expect(report.anomalies).toEqual(expect.arrayContaining([expect.stringContaining('not-a-date')]));
    });
    it('写出 date.json、date.md、date.html，且 HTML 不依赖网络资源', async () => {
        ...
        const jsonContent = JSON.parse(await readFile(path.join(outputRoot, '2026-08-16.json'), 'utf8'));
    ...
    it('拒绝不安全的报告日期，且不会在 outputRoot 外写文件', async () => {
        ...
        await expect(writeReportBundle({
            date: unsafeDate,
            ...
            failures: [],
            anomalies: [],
        }, outputRoot)).rejects.toThrow(/date|safe|invalid/i);
    ...
    it('将 javascript 岗位链接渲染为不可点击文本并保留安全 HTTPS 链接', async () => {
        ...
        ], confirmedProfile, '2026-08-16');
        await writeReportBundle(report, outputRoot);
        const htmlContent = await readFile(path.join(outputRoot, '2026-08-16.html'), 'utf8');
        expect(htmlContent).toContain('URL: javascript:alert(1)');
        expect(htmlContent).not.toMatch(/href\s*=\s*["']javascript:/i);
        expect(htmlContent).toContain('<a href="https://jobs.example.com/roles/safe">https://jobs.example.com/roles/safe</a>');
    });
});
//# sourceMappingURL=daily-report.test.js.map