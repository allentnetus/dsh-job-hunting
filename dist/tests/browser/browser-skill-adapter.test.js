...
import { BrowserHumanAssistanceRequiredError, checkBrowserSkill, collectWithBrowserSkill, } from '../../src/browser/browser-skill-adapter.js';
class FakeBskRunner {
    results;
    onRun;
    commands = [];
    constructor(results, onRun) {
...
describe('BrowserSkill adapter', () => {
    ...
    it('reports a clear cleanup error when collection succeeds but session cleanup fails', async () => {
        ...
        await expect(collectWithBrowserSkill(request, runner)).rejects.toThrow(/cleanup.*stop failed/i);
    });
    ...
    it('reports human assistance for login or captcha without retrying', async () => {
        ...
        await expect(collectWithBrowserSkill(request, runner)).rejects.toBeInstanceOf(BrowserHumanAssistanceRequiredError);
...
//# sourceMappingURL=browser-skill-adapter.test.js.map