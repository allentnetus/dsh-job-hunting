...
vi.mock('node:fs/promises', async (importOriginal) => {
    ...
    fsPromisesMock.lstat.mockImplementation((...args) => actual.lstat(...args));
    fsPromisesMock.rename.mockImplementation((...args) => actual.rename(...args));
    fsPromisesMock.stat.mockImplementation((...args) => actual.stat(...args));
    return {
        ...actual,
...
import { createOrUpdateShortcut, ShortcutOwnershipError, validateShortcutTarget, } from '../../src/desktop/desktop-shortcut.js';
...
const makeRequest = (siteRoot, overrides = {}) => ({
    siteRoot,
    name: 'Job Hunting',
    approved: true,
...
describe('windows shortcut ownership and approval', () => {
    ...
    it.each([
        ...
        await expect(createOrUpdateShortcut(makeRequest(siteRoot), shell)).rejects.toBeInstanceOf(ShortcutOwnershipError);
        expect(shell.createShortcut).not.toHaveBeenCalled();
        expect(shell.updateShortcut).not.toHaveBeenCalled();
    ...
    it('performs zero shell writes when approval is false', async () => {
        ...
        await expect(createOrUpdateShortcut(makeRequest(siteRoot, { approved: false }), shell)).resolves.toMatchObject({ status: 'approval-required' });
        expect(shell.createShortcut).not.toHaveBeenCalled();
        expect(shell.updateShortcut).not.toHaveBeenCalled();
        expect(shell.writeManifestAtomically).not.toHaveBeenCalled();
    });
...
describe('Windows manifest atomic-write fallback', () => {
    ...
    it('refuses to direct-overwrite a symlink destination when atomic rename is unavailable', async () => {
        ...
        fsPromisesMock.rename.mockRejectedValueOnce(Object.assign(new Error('atomic replace unavailable'), { code: 'EPERM' }));
...
//# sourceMappingURL=windows-shortcut.test.js.map