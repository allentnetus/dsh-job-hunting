...
const assertFilesExist = async (relativePaths) => {
    await Promise.all(relativePaths.map(async (relativePath) => {
        await access(path.join(root, relativePath));
    }));
};
const assertIssueFormShape = (content, relativePath) => {
...
const walkReleaseFiles = async (directory) => {
...
describe('release checks', () => {
    ...
    it('documents the DeepSeek Harness installation and profile override path', async () => {
        ...
        const readme = await readText('README.md');
        const browser = await readText('docs/browser-skill-integration.md');
        expect(installation).toContain('dsh plugin --profile web add');
        expect(installation).toContain('dsh --profile web --dump-config');
    ...
    it('has no obvious secrets, resume files, or retired branding in release-oriented files', async () => {
        ...
        const contents = await Promise.all(relativePaths.map(async (relativePath) => ({
            relativePath,
            content: await readText(relativePath),
        })));
        const obviousSecret = /-----BEGIN (?:RSA |OPENSSH |EC |PGP )?PRIVATE KEY-----|\b(?:sk|ghp|github_pat|xox[baprs])-[A-Za-z0-9_-]{12,}\b|\bAKIA[0-9A-Z]{16}\b/;
        ...
        for (const { relativePath, content } of contents) {
            ...
            expect(relativePath.toLowerCase(), relativePath).not.toMatch(/(?:^|[\\/])(?:resume|简历)[^\\/]*\.(?:docx|pdf|txt|md)$/);
        }
    ...
    it('keeps package entrypoints, templates, documentation, and dry-run contents aligned', async () => {
        ...
        expect(packageJson.scripts?.['release:check']).toBe('corepack pnpm build && vitest run tests/release/release-checks.test.ts && corepack pnpm dsh:smoke && corepack pnpm pack --dry-run');
        await assertFilesExist([
            'package.json',
            'dist/src/index.js',
            'dist/src/index.d.ts',
...
//# sourceMappingURL=release-checks.test.js.map