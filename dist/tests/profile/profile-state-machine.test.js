...
const loadResumeAssessorModule = async () => import('../../src/resume/resume-assessor.js').catch(() => undefined);
const loadProfileStateMachineModule = async () => import('../../src/profile/profile-state-machine.js').catch(() => undefined);
const loadCareerProfileModule = async () => import('../../src/profile/career-profile.js').catch(() => undefined);
const createParsedResume = (overrides = {}) => ({
...
describe('profile state machine', () => {
    it('按 parsed -> user-confirmed -> assessed -> profile-draft -> profile-confirmed 流转，并分开保存用户修正与模型建议', async () => {
        ...
        const assessedResult = stateMachineModule.applyResumeAssessment(confirmedResult.value, assessment);
    ...
    it('拒绝重复确认简历与非法确认 profile draft', async () => {
        ...
        expect(stateMachineModule.confirmResumeDocument(createParsedResume({
            state: 'user-confirmed',
            confirmation: {
                confirmedAt: '2026-08-16T09:00:00.000Z',
                corrections: [],
        ...
        expect(stateMachineModule.confirmProfileState({
            state: 'assessed',
            targetRoles: [],
            targetIndustries: [],
            targetCompanies: [],
...
//# sourceMappingURL=profile-state-machine.test.js.map