...
const getExpectedBuildArtifact = (rootDir, outDir, sourcePath, extension) => {
...
describe('项目元数据', () => {
    ...
    it('声明任务 1 要求的脚本、DSH peer 版本，并且不把 BrowserSkill 作为 npm 依赖', async () => {
        ...
        expect(declaredDependencyNames.some((dependencyName) => /browser[-_]?skill/i.test(dependencyName))).toBe(false);
    });
    ...
    it('让 package.json 和 README 中的入口声明与 tsconfig 构建结构一致', async () => {
        ...
        const runtimeSkillJs = getExpectedBuildArtifact(rootDir, outDir, 'src/skill/job-hunting.skill.ts', '.js');
        const runtimeSkillTypes = getExpectedBuildArtifact(rootDir, outDir, 'src/skill/job-hunting.skill.ts', '.d.ts');
