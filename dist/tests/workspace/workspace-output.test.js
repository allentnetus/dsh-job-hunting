...
import { ensureOutputTree, readWorkspaceJson, resolveActiveWorkspace, resolveOutputRoot, writeWorkspaceJson, } from '../../src/workspace/workspace-output.js';
const tempDirs = [];
...
const createRegistry = (workspaces, resolveByPath) => ({
    get: vi.fn((id) => workspaces.find((workspace) => workspace.id === id)),
    list: vi.fn(() => [...workspaces]),
    resolveByPath: vi.fn(resolveByPath ??
        (async (cwd) => workspaces.find((workspace) => workspace.path === cwd))),
});
describe('workspace-output', () => {
    ...
    it('当 sessionId 找不到时，回退到 session.meta.cwd + resolveByPath', async () => {
        ...
        const registry = createRegistry([workspace], async (cwd) => cwd === workspace.path ? workspace : undefined);
        const ctx = {
            agent: {
    ...
    it('只允许在 workspace 内安全拼接相对输出路径', () => {
        ...
        expect(resolveOutputRoot(workspace, 'job-hunting-site/reports')).toBe(path.resolve(workspace.path, 'job-hunting-site', 'reports'));
    ...
    it('用同目录临时文件写入 JSON，并能安全读回', async () => {
        ...
        await expect(readWorkspaceJson(root, 'config/profile.json')).resolves.toEqual(payload);
...
//# sourceMappingURL=workspace-output.test.js.map