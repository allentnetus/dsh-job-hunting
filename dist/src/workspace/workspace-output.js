...
const normalizeWorkspace = async (workspace) => ({
    ...
    status: typeof workspace.status === 'function'
        ? await workspace.status()
        : workspace.status ?? 'ok',
});
const requireWorkspaceRegistry = (ctx) => {
...
const resolveRelativePath = (root, relativePath) => {
...
export const resolveActiveWorkspace = async (ctx) => {
...
export const resolveOutputRoot = (workspace, relativePath) => resolveRelativePath(workspace.path, relativePath);
export const ensureOutputTree = async (root) => {
    const directories = [
...
export const readWorkspaceJson = async (root, relativePath) => {
...
export const writeWorkspaceJson = async (root, relativePath, value) => {
    const filePath = resolveRelativePath(root, relativePath);
...
//# sourceMappingURL=workspace-output.js.map